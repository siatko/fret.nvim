...
describe("editor", function()
  ...
  describe("FretCopy", function()
    it("puts content wrapped in a fret code block on the clipboard", function()
      open_test_song()
      vim.cmd("FretCopy")
      local clip = vim.fn.getreg("+")
      assert.truthy(clip:match("^```fret\n"), "should start with ```fret")
      assert.truthy(clip:match("\n```$"),     "should end with ```")
    end)

    it("clipboard content contains the song title", function()
      open_test_song("Clip Song")
      vim.cmd("FretCopy")
      local clip = vim.fn.getreg("+")
      assert.truthy(clip:find("Clip Song", 1, true))
    end)

    it("clipboard content matches the buffer content", function()
      open_test_song()
      vim.cmd("FretCopy")
      local clip  = vim.fn.getreg("+")
      local inner = clip:match("^```fret\n(.*)\n```$")
